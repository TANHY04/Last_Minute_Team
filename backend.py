...
def loadTransactionDataDf():
    #transactionDataDf
    transactionDataDf=pd.read_csv('C:/Users/gztan/Datasets/transaction_data.csv', encoding='utf-8')
    transactionDataDf['order_time'] = pd.to_datetime(transactionDataDf['order_time'])
    transactionDataDf['order_day'] = transactionDataDf['order_time'].dt.strftime('%d/%m/%Y')
...
if merchantID:
    if todayDate:
        ...
        def analyzeDailySales(merchantID, todayDate, numResults):    
            ...
            # Aggregate
            sales_by_time = merchantDateDf.groupby('sales_time', as_index=False)['order_value'].sum()
            sales_by_time = sales_by_time.sort_values('sales_time')

            # Plot
            fig = px.line(sales_by_time, x='sales_time', y='order_value',
                        labels={'sales_time': 'Sales Time (1-hour slots)', 'order_value': 'Total Order Value'},
            ...
            merchantDateDf['total_sales'] = merchantDateDf.groupby('sales_time')['order_value'].transform('sum')
            top_sales_times = (
        ...
        def analyzeMostOrderedFood(merchantID, todayDate, numMostOrderedFood=5):
            ...
            merchantDateDf['order_day'] = pd.to_datetime(merchantDateDf['order_day'], format="%d/%m/%Y")
            ...
            merchantDateDf['sales_time'] = (
                rounded_time.dt.strftime('%H%M') + '-' + (rounded_time + pd.Timedelta(hours=1)).dt.strftime('%H%M')
            )
            merchantDateDf['order_day'] = merchantDateDf['order_time'].dt.strftime('%d/%m/%Y')
            ...
            merged_df = pd.merge(transactionItemsDf, merchantDateDf[['order_id', 'sales_time']], on='order_id', how='inner')

            # Step 5: Frequency calculation
            freq_df = merged_df.groupby(['sales_time', 'item_id']).size().reset_index(name='frequency')

            # Step 6: Clustering on frequency
            if len(freq_df) >= 3:
                kmeans = KMeans(n_clusters=3, random_state=42)
                freq_df['cluster'] = kmeans.fit_predict(freq_df[['frequency']])
                most_freq_cluster = freq_df.groupby('cluster')['frequency'].mean().idxmax()
            ...
            itemsDf = loadItemsDf()  # Make sure this function returns your items metadata with item_id + item_name
            id_to_name = dict(zip(itemsDf['item_id'], itemsDf['item_name']))
            filtered_freq_df['item_name'] = filtered_freq_df['item_id'].map(id_to_name)
            filtered_freq_df['display_name'] = filtered_freq_df['item_name'] + ' (' + filtered_freq_df['frequency'].astype(str) + ')'

            # Step 8: Sort and select top N items per time block
            sorted_items = filtered_freq_df.sort_values(by=['sales_time', 'frequency'], ascending=[True, False])
            top_items = (
                sorted_items
                .groupby('sales_time')
            ...
            padded_lists = {time: lst + [None] * (max_len - len(lst)) for time, lst in top_items.items()}
            final_df_named = pd.DataFrame.from_dict(padded_lists, orient='index').transpose()
        ...
        #5) Analyze Daily Sales
        st.subheader('5) Analyze daily sales')
        numResultsSalesView= st.text_input("Enter number of results:", key="numResultsSalesView")
        if(numResultsSalesView):
            analyzeDailySales(merchantID, todayDate,numResultsSalesView)
        else:
            analyzeDailySales(merchantID, todayDate,5)
        salesAtTimeSlotView= st.text_input("Enter time to check sales:", key="salesAtTimeSlotView")
        if(salesAtTimeSlotView):
            printSalesAtTimeSlotDf(merchantID, todayDate, salesAtTimeSlotView)
        
        #6) Analyze Most Ordered Food in  each time period
        st.subheader('6) Analyze most ordered food in each time period')
        numMostOrderedFood= st.text_input("Enter number of results:", key="numMostOrderedFood")
        if(numMostOrderedFood):
            analyzeMostOrderedFood(merchantID,todayDate,numMostOrderedFood)
        else:
            analyzeMostOrderedFood(merchantID,todayDate,5)